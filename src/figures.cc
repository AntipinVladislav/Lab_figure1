...
FigureArray::FigureArray(const FigureArray& arr) {
	for (int i = 0; i < Size(); ++i) {
		figure_array.push_back(arr[i]->Clone());
	}
}

void FigureArray::DeleteFigure(int index) {
	auto iter = figure_array.cbegin();
	figure_array.erase(iter + index);
}

int FigureArray::Size() {

	return figure_array.size();
}

void FigureArray::Insert(FigurePtr figure, int index) {
	auto iter = figure_array.cbegin();
	figure_array.emplace(iter + index, figure);
}
...
Figure& FigureArray::MinSquareSearch() {
	int j = 0;
	for (int i = 1; i < figure_array.size(); ++i)
		if ((figure_array[j]->GetSquare()) > (figure_array[i]->GetSquare()))
			j = i;
	return *figure_array[j];
}

void FigureArray::Print() {
	for (int i = 0; i < Size(); ++i) {
		cout << i << ". "; figure_array[i]->Print();
		cout << endl;
	}
}